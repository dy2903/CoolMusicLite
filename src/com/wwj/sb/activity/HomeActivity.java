...
//		自定义菜单
		xmenu = new Menu(this);
//		data1是一个List , 里面的子项是长度为2的数组,0表示按键, 1表示名称
		...
		xmenu.addItem("工具", data2, new MenuAdapter.ItemListener() {
			...
			public void onClickListener(int position, View view) {
				...
				if (position == 0) {
					...
					HomeActivity.this.Sleep();
				} else if (position == 2) {
					HomeActivity.this.setBrightness(view);
				}
			}
		});
//		帮助
...
//		this.getMenuInflater().inflate (R.menu.main , menu);
	...
	/**
	 * 处理startActivityForResult()的返回值.
	 * 当SkinSettingActivity关闭的时候返回
	 * requestCode = 2
	 */
	@Override
    ...
    */
	...
	private class MusicListItemClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			HomeActivity.this.playMusic(listPosition); // 播放音乐
		}
	...
	public class MusicListItemContextMenuListener implements
		...
		public void onCreateContextMenu(ContextMenu menu, View v,
			...
			HomeActivity.this.musicListItemDialog(); // 长按后弹出的对话框, 是一个List
...
//		显示Dialog
		...
		menuList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (position == 0) {
					HomeActivity.this.playMusic(listPosition);
				} else if (position == 1) {
					setRing();
				} else if (position == 2) {
					HomeActivity.this.showMusicInfo(listPosition);
				}
			}
...
//			获取专辑封面位图对象
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
	 ...
	 /**
	  * 自定义的BroadcastReceiver，负责监听从Service传回来的广播
	  * 根据Intent传递过来的信息,改变背景 或者时间和标题的更新
	  */
